...
export async function getOrSetCart(countryCode: string) {
  ...
  // Eğer mevcut sepet daha önce tamamlanmış bir siparişe aitse (Quick Checkout sonrası), çerezi silip yeni sepet oluşturalım.
...
export async function addGiftToCart({
  variantId,
  countryCode,
}: {
  variantId: string
  ...
  const backendUrl = process.env.NEXT_PUBLIC_MEDUSA_BACKEND_URL || process.env.MEDUSA_BACKEND_URL || "https://api.cizgibutik.com"
  
  const response = await fetch(`${backendUrl}/store/custom/kampanyalar/add-gift`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      cart_id: cart.id,
...
export async function clearQuickCheckoutCart() {
  try {
    await removeCartId()
    const cartCacheTag = await getCacheTag("carts")
    revalidateTag(cartCacheTag)
  } catch (e) {
    console.error("Error clearing cart after quick checkout", e)
  }
}
